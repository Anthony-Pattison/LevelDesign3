...
namespace CoverShooter
{
    public class BrainEditor : EditorWindow
    {
        ...
        [NonSerialized]
        private Brain _lastBrain;

        [NonSerialized]
        private string _lastBrainText;

        [NonSerialized]
        private bool _hasSaveTimer;
        ...
        private static void Create()
        {
            ...
            File.WriteAllText(path, JsonUtility.ToJson(new Brain()));
            AssetDatabase.ImportAsset(path);
        }

        public void IgnoreBrainChange()
        {
            _hasSaveTimer = false;
            _lastBrain = Brain;

            if (Brain != null)
                _lastBrainText = JsonUtility.ToJson(Brain);
        }
        ...
        private void OnGUI()
        {
            ...
            if (Brain != null && (Brain.Layers == null || Brain.Layers.Length == 0))
            {
                Brain.AddLayer("Base");
            ...
            _horizontalSplit.Begin(350, float.MaxValue);
            {
                ...
                if (_panel.Display(Brain, _renamer, ref _activeLayer))
                    _needsRepaint = true;
            ...
            if (_lastBrain != Brain && _lastBrain != null)
                save(_lastBrain);
            ...
            if (_needsRepaint)
            {
                ...
                if (_hasSaveTimer)
                {
                    if (DateTime.Now > _saveTime.AddSeconds(2))
                    {
                        save(Brain);
                        _hasSaveTimer = false;
                    }
                ...
                {
                    var text = JsonUtility.ToJson(Brain);

                    if (_lastBrain == Brain && _lastBrainText != text)
                    {
                        _hasSaveTimer = true;
        ...
        private void OnSelectionChange()
        {
            if (Selection.activeObject != null)
            {
                ...
                if (Selection.activeObject is Brain)
                    value = (Brain)Selection.activeObject;
                else if (Selection.activeObject is AIController)
                {
                    Controller = (AIController)Selection.activeObject;
                    value = Controller.Brain;
                }

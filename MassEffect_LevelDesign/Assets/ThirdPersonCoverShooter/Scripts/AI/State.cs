...
namespace CoverShooter.AI
{
    ...
    public class State
    {
        ...
        public void SetTargetAt(GameObject value, Vector3 position)
        {
            ...
            if (Vector3.Distance(TargetPosition, position) > 0.001f)
            {
                Feed(AIEvent.NewTargetPosition);
        ...
        public bool IsActive(int layer, int id)
        {
            if (Layers == null || layer < 0 || layer >= Layers.Length)
                return false;

            return Layers[layer].CurrentNode == id;
        }
        ...
        public bool Trigger(int layer, ActionNode parent, NodeTrigger trigger)
        {
            ...
            if (trigger.Target == ExitID)
            {
                ...
                var parentTriggerId = parent.SuccessTrigger;
                var parentTrigger = _brain.GetNodeTrigger(parentTriggerId);

                if (parentTrigger == null || parentTrigger.Target == 0)
                    return false;

                var triggerId = GetNodeTriggerId(trigger);

                if ((triggerId != 0 && triggerId == parentTriggerId) || parentTrigger == trigger)
                    return false;

                trigger.EditorDebugValue = 1;
                parentTrigger.EditorDebugValue = 1;

                var grandParent = parent != null ? _brain.GetAction(parent.Parent) : null;
                return Trigger(layer, grandParent, parentTrigger);
            }
            else if (trigger.Target == FailID)
            {
                ...
                var parentTriggerId = parent.FailureTrigger;
                var parentTrigger = _brain.GetNodeTrigger(parentTriggerId);
        ...
        private int GetNodeTriggerId(NodeTrigger trigger)
        {
            if (_brain == null || trigger == null)
                return 0;

            foreach (var pair in _brain.NodeTriggers)
